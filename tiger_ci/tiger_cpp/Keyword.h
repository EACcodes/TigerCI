...
// ref vecs keywords are specific to the storage of vectors of references
class RefVecsKeyword : public Keyword{
public:
    // constructor requires keyword name
    RefVecsKeyword(string name);
    
    // specific parsing for reference keywords
    void parse(ifstream& input);
    
    // returns a reference to the vector of reference vectors
    vector<vector<int>>& getRefVecsKeyword();
    
    // run a check for reference consistency, given several other input parameters
    void RefVecsCheck(int num_ref, int num_electrons, int num_inactive,
                      int num_frozen);
protected:
    // value of the reference vector of vectors
    vector<vector<int>> _val;
};

// cyl vecs keywords are specific to the storage of vectors containing manual
// cylinder coordinates
class CylVecsKeyword : public Keyword{
public:
    // constructor requires keyword name
    CylVecsKeyword(string name);
    
    // specific parsing for cylinder coordinates
    void parse(ifstream& input);
    
    // returns a reference to the vector of cylinder coordinate vectors
    vector<Chain>& getCylVecsKeyword();
    
protected:
    // value of the cylinder coordinate vector of vectors
    vector<Chain> _val;
};
