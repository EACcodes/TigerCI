...
benchmark_energy=-114.94154080                 # from the CASSCF calculation w/ MOLCAS
...
echo " "
echo "$Project-super-ref-test" 
echo "*********************************"
$TIGER/tiger_ci.exe $WorkDir/$Project.input > $WorkDir/$Project.output 2>&1 

# Test the result
echo "testing the final energy"
python $ThisDir/error-check.py $benchmark_energy $WorkDir/$Project.output '1.0E-6'
if [ $? -ne 0 ]
then
    exit 2
fi
